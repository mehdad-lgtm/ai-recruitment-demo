  ...
  return (
    <ProtectedDashboard allowedRoles={["admin"]}>
      ...
      <div className="grid grid-cols-3 gap-6 mb-6">
        ...
        <div className="col-span-2 bg-card rounded-xl border border-border overflow-hidden">
          <div className="p-4 border-b border-border">
            <h2 className="text-lg font-semibold text-foreground">Recruitment Funnel</h2>
            <p className="text-sm text-muted-foreground">Candidate journey through the pipeline</p>
          </div>
          <div className="p-4 space-y-3">
            {conversionStats.map((stage, index) => (
              <div key={stage.stage} className="flex items-center gap-4">
                <div className="w-48 text-sm text-foreground truncate">{stage.stage}</div>
                <div className="flex-1">
                  <div className="h-8 bg-muted rounded-full overflow-hidden">
                    <div
                      className="h-full bg-gradient-to-r from-primary to-primary/60 rounded-full flex items-center justify-end pr-3"
                ...
                <div className="w-16 text-right">
                  <span className="text-sm font-medium text-foreground">{stage.percentage}%</span>
                </div>
              </div>
            ))}
          </div>
            ...
            {recentActivity.map((activity, index) => (
              <div key={index} className="flex items-start gap-3">
                <div className={cn(
                  ...
                  {activity.type === "hire" && <UserCheck className="h-4 w-4 text-green-500" />}
                  {activity.type === "interview" && <Calendar className="h-4 w-4 text-blue-500" />}
                  {activity.type === "qr" && <QrCode className="h-4 w-4 text-purple-500" />}
                  {activity.type === "communication" && <MessageSquare className="h-4 w-4 text-orange-500" />}
                </div>
      ...
      <div className="grid grid-cols-2 gap-6">
        ...
        <div className="bg-card rounded-xl border border-border overflow-hidden">
          <div className="p-4 border-b border-border">
            ...
            <p className="text-sm text-muted-foreground">Candidate sources and conversion rates</p>
          </div>
        ...
        <div className="bg-card rounded-xl border border-border overflow-hidden">
          <div className="p-4 border-b border-border">
            <h2 className="text-lg font-semibold text-foreground">Interviewer Performance</h2>
            <p className="text-sm text-muted-foreground">Interview metrics by team member</p>
          </div>
          <div className="overflow-x-auto">
            <table className="w-full">
              <thead className="bg-muted/50">
      ...
      <div className="mt-6 bg-card rounded-xl border border-border overflow-hidden">
        <div className="p-4 border-b border-border">
          <h2 className="text-lg font-semibold text-foreground">AI Communication Performance</h2>
          <p className="text-sm text-muted-foreground">Automated chat and voice interaction metrics</p>
        </div>
        <div className="grid grid-cols-5 divide-x divide-border">
          <div className="p-6 text-center">
            <p className="text-3xl font-bold text-foreground">892</p>
            <p className="text-sm text-muted-foreground mt-1">Chats Handled</p>
          </div>
          <div className="p-6 text-center">
            <p className="text-3xl font-bold text-foreground">156</p>
            <p className="text-sm text-muted-foreground mt-1">Voice Calls</p>
          </div>
          <div className="p-6 text-center">
